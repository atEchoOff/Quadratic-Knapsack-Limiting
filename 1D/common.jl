function rhs_low_no_logic!(du, u, cache, t)
    (; physics, rd, md) = cache
    ...
    for e in 1:md.num_elements        
        ...
        function f(x)
            x = reinterpret(SVector{3, Float64}, x)
            return [-sum(dot.(v, x)), sum(x)...]
        end

        what_to_achieve = [-4000abs(sum(cache.B * psi.(u_element, equations))), 0.0, 0.0, 0.0]

        returned = min_norm_solution(f, what_to_achieve, length(rhs_vol_high) * 3)

        rhs_vol_low = reinterpret(SVector{3, Float64}, returned)

        # optimization target
        b = sum(cache.B * psi.(u_element, equations)) + sum(dot.(v, rhs_vol_low))

        @assert norm(f(rhs_vol_low) - what_to_achieve) < 1e-6

        # We aim for b > 0
        @assert b >= -1e-6

        # We also want conservation
        @assert all(abs.(sum(rhs_vol_low)) .<= 1e-6)

        # So what happens when we aim for the minimum norm so that b = 0 and conservation is achieved?
...
function rhs_min_norm!(du, u, cache, t)
    (; physics, rd, md) = cache
    (; Q_skew) = cache.high_order_operators
    (; Q_skew_low) = cache.low_order_operators    
    (; Δ, R) = cache.fv_operators
    ...
    for e in 1:md.num_elements        
        ...
        if -sum(dot.(v, rhs_vol_high)) < sum(cache.B * psi.(u_element, equations)) + 100 * eps()
            # High order is ES
            view(du, :, e) .+= rd.M \ rhs_vol_high
            continue
        end
        ...
        what_to_achieve = [sum(cache.B * psi.(u_element, equations)) - (-sum(dot.(v, rhs_vol_high))), 0.0, 0.0, 0.0]

        returned = min_norm_solution(f, what_to_achieve, length(rhs_vol_high) * 3)

        difference = reinterpret(SVector{3, Float64}, returned)

        rhs_vol_high .+= difference

        # @assert abs(sum(cache.B * psi.(u_element, equations)) - sum(dot.(v, rhs_vol_high))) < 1e-6
...
function rhs_no_idea!(du, u, cache, t)
    (; physics, rd, md) = cache
    (; Q_skew) = cache.high_order_operators
    (; Q_skew_low) = cache.low_order_operators    
    (; Δ, R) = cache.fv_operators
    ...
    for e in 1:md.num_elements        
        ...
        # optimization target
        rhs_vol_low .= -(u_element .- [sum(u_element) / length(u_element)])
